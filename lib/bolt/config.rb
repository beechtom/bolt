  ...
  class Config
    ...
    def config_loaded
      msg = <<~MSG
        ...
        Bolt loads configuration files at three different levels (project, user, system-wide). From
        highest precedence to lowest, the configuration files currently loaded are:

          #{[*@config_files, boltdir.config_file].reverse.join("\n  ")}

        CONFIGURATION MERGE STRATEGY

        When merging configuration, Bolt's strategy is to shallow merge any options that accept
        hashes and to overwrite any options that do not accept hashes. There are two exceptions to
        this strategy:

          - TRANSPORT CONFIGURATION [#{TRANSPORTS.keys.map(&:to_s).join(', ')}]

            Transport configuration options are deep merged.
          ...
          - PLUGIN CONFIGURATION [plugins]
            ...
            The 'plugins' option accepts a hash where each key is the name of a plugin and its value is
            a hash of configuration options for the plugin. When merging configuration, the configuration
            for individual plugins is shallow merged.

            If a plugin is configured in one file, but is not configured in a file with a higher precedence,
            the configuration for the plugin will still be present in the merged configuration.
        ...
        More information about configuring Bolt and configuration file paths can be found in the
        Bolt documentation: https://puppet.com/docs/bolt/latest/configuring_bolt.html
      MSG
